...
- **Usage analytics** — cumulative cost + input/output tokens per session, plus
  a live rate-limit readout refreshed every minute: **percentages straight from
  the CLI's own `/usage` report** (session and week), plus a ticking countdown to
  the reset. If that report can't be read, it falls back to showing the tokens
  actually spent inside the window rather than inventing a figure
